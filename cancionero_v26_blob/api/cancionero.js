  ...
  try {
    if (req.method === 'POST') {
      ...
      if(action==='upload-event-photo'){
        ...
        const meta={id:`photo-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`,userId:session.userId,mote:safeText(session.user.mote,80),caption:safeText(req.body?.caption,160),createdAt:new Date().toISOString()};const savedPhoto=await addPhoto(eventId,meta,imageData);return res.status(200).json({ok:true,photo:{...savedPhoto,blobUrl:undefined,pathname:undefined},myPhotoCount:mine.length+1});
      }
