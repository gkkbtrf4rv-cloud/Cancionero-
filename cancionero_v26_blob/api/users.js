...
import { sendEmail, approvedEmail, getAppUrl, emailConfigStatus } from '../lib/email.js';
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'POST' && req.body?.action === 'test-email') {
      ...
      if (!config.configured) {
        return res.status(400).json({ error:'Falta configurar GMAIL_USER o GMAIL_APP_PASSWORD en Vercel.', config });
      }
