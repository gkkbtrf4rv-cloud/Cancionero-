...
export function cleanEvent(input={}, existing=null){
  ...
  return {
    ...
    setlist: Array.isArray(input.setlist) ? input.setlist.map(x=>cleanText(x,90)).filter(Boolean).slice(0,120) : (Array.isArray(existing?.setlist)?existing.setlist:[]),
...
export async function getEventSummaries({includeHidden=false}={}){
  ...
  return Promise.all(events.map(async e=>{
    ...
    const coverImageUrl=e.coverPathname?`/api/cancionero?asset=cover&eventId=${encodeURIComponent(e.id)}&v=${encodeURIComponent(e.coverUpdatedAt||'1')}`:null;
...
export async function getPhotoMeta(eventId, photoId){
  const index=await getPhotoIndex(eventId);
...
export async function getPhotoPage(eventId, offset=0, limit=12){
  ...
  const items=await Promise.all(slice.map(async meta=>{
    ...
    if(meta?.storage==='vercel-blob-private' && meta.pathname){
      return {...meta, imageUrl:`/api/cancionero?asset=photo&eventId=${encodeURIComponent(eventId)}&photoId=${encodeURIComponent(meta.id)}`};
    }
