...
export default async function handler(req, res) {
  ...
  try {
    ...
    const oldMeta = await kv.get(`submeta:${id}`);
    await kv.set(`sub:${id}`, subscription);
    await kv.set(`submeta:${id}`, {
      id, userId:session.userId, email:session.user.email || session.user.username || '', mote:session.user.mote,
      userAgent:String(device.userAgent || req.headers['user-agent'] || '').slice(0,300),
      platform:String(device.platform || '').slice(0,100), installed:Boolean(device.installed),
      createdAt:oldMeta?.createdAt || now, lastSeenAt:now, lastDelivery:oldMeta?.lastDelivery || null
    });
